...
\newcommand{\parMinStableStepSize}{\beta_{\text{stb}}^{\min}}
...
\newcommand{\parInitializeMin}{\beta_{10}}
\newcommand{\parInitializeMinValue}{10^{-1}}
\newcommand{\parInitializeMinInterval}{(0,\infty)}

\newcommand{\parInitializeMax}{\beta_{11}}
\newcommand{\parInitializeMaxValue}{10^{3}}
\newcommand{\parInitializeMaxInterval}{[\parInitializeMin,\infty)}

\newcommand{\parAggProtect}{\beta_{12}}
\newcommand{\parAggProtectValue}{0.9}
\newcommand{\parAggProtectInterval}{(0,1)}
...
Finally, we note that we improving the accuracy of the linear system solves would also improve our robustness. A disadvantage of using the Cholesky factorization is that we often had difficulty obtaining a sufficiently accurate solution as we approached optimality. Potentially switching to an \LBL{} factorization \cite{amestoy1998mumps} or other techniques [REF] might help resolve this issue.
...
The remainder of the initialization scheme is inspired by Mehrotra's start scheme for linear programming \cite{mehrotra1992implementation}, adapted to the nonlinear programming context. We select a candidate dual and slack variables from
\begin{flalign*}
...
\tilde{y} &\gets \tilde{y} + \varepsilon_{y} \\
 \varepsilon_{s} &\gets \max\left\{  - 2 \min_i{ \tilde{s}_i}, 0 \right\} \\
...
\begin{flalign*}
\tilde{s} &\gets \tau (a(x^0) + \tilde{s}) - a(x^0)  \\
\tilde{\mu} &\gets \tau \tilde{\mu}.
\end{flalign*}
Next, set
\begin{flalign*}
\mu^0 &\gets \parMuScale \tilde{\mu} \\
s^0_i &\gets \tilde{s}_i \quad \forall i \not\in \mathcal{B}  \\
s^0_i &\gets -\cons_i(x^0) \quad \forall i \in \mathcal{B}  \\
