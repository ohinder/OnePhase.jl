...
The infeasible-start method for linear programming \cite{lustig1990feasibility}, naturally extends to non linear optimization. 
...
d_{x}^{*} \in \arg \min_{\bar{d}_{x} \in \R^{\nvar}} & \psi_{\mu (1 - \eta)}(x + \bar{d}_{x}) + \frac{\delta}{2} \| \bar{d}_{x} \|^2 %\label{prox-obj}
\end{flalign}
where $\eta \in [0,1]$ represents the target reduction in constraint violation and barrier parameter $\mu$; and $\delta > 0$ scales the size of the proximal term. Primal interior point methods \cite{fiacco1990nonlinear} apply Newton's method directly to system \eqref{sophisticated-barrier-problem}. However, they have inferior practical performance to primal-dual methods, that apply Newton's method directly to the optimality conditions. To derive the primal-dual directions let us write the first-order optimality conditions
\begin{flalign*}
\nabla_{x} \Lag(x + d_{x}^{*}, y + d_{y}^{*}) + \delta d_{x}^{*} &=  (1 - \eta) \mu \nabla \regularizer( x + d_{x}^{*} )  \\
a(x + d_{x}^{*}) + s + d_{s}^{*} &= (1 - \eta) \mu \conWeight \\
(S^{*} + D_{s}^{*}) (y + d_{y}^{*}) &= (1 - \eta) \mu e \\
s + d_{s}^{*}, y + d_{y}^{*} &\ge 0
\end{flalign*} 
...
This is similar to the system typically factorized by non linear programming solvers using LDL \hinder{citations}. However, due to the $\hat{Y}^{-1} \hat{S}$ block we can further reduce the system. By taking the primal schur complement we see solving \eqref{primal-dual-Newton-direction} for $d_{x}$ is equivalent to solving:
...
Note that if the matrix $(\schur + \delta I)$ is positive definite, $\eta = 0$ and $(\hat{x},\hat{y},\hat{s}, \hat{\mu}) = (x,y,s, \mu)$ then the right-hand side of \eqref{eq:schur-complement} becomes $-\nabla \barrier_{\mu}(x)$ hence  $d_{x}$ is a descent direction for the function $\barrier_{\mu}(x)$. Consequently, during our algorithm we pick $\delta > 0$ such that $\schur + \delta I$ is positive definite. Aside from performing a Cholesky factorization of $\schur + \delta I$, we can also determine that the matrix is positive definite by computing the number of positive and negative elements in the diagonal of the LDL factorization of \eqref{eq:ldl-system}. A more indepth discussion of the linear algebra details will be given in Section~\ref{sec:linear-algebra}.
...
It remains to describe how to update the dual variables. Given some candidate primal iterate $(x^{+}$, $s^{+})$,  let $B( s^{+}, d_{y} )$ be the set of feasible dual step sizes. More precisely, $B( s^{+}, d_{y} ) \subseteq [0,1]$ is the largest interval such that if $\alpha_{D} \in B( s^{+}, d_{y} )$ then
%\begin{subequations}
\begin{flalign}\label{subeq:set-B}
...
\end{flalign}
%\end{subequations}
If there is no value of $\alpha_{D}$ satisfying \eqref{subeq:set-B} we set $B( s^{+}, d_{y} )$ to the empty set and the step will be rejected. Recall the parameter $\parComp \in (0,1)$ was defined in \eqref{eq:comp-slack}. The purpose criteria \eqref{subeq:set-B} is to ensure we always satisfy \eqref{eq:comp-slack}, i.e., $\frac{S y}{\mu} \in [e \parComp, e/\parComp ]$. 

 We compute the dual step size as follows:
 
 \hinder{Am currently experimenting with different line searches}
\begin{flalign}
\alpha_{D} = \arg \min_{\gamma \in B( s^{+}, d_{y} )} \| S^{+} y - \mu^{+} + \gamma S^{+} d_{y} \|^2_{2} + \| \nabla_{x} \Lag(x, y)  + (\nabla_{x}^2 \Lag(x, y) + \delta I) \alpha_{P} d_{x}  + \gamma \nabla a(x)^T d_{y} \|^{2}_{2}.
\label{eq:alphaD-least-squares}
\end{flalign}

%\begin{flalign}
%\alpha_{D} = \arg \min_{\gamma \in B( s^{+}, d_{y} )} \| S^{+} y - \mu^{+} + \gamma S^{+} d_{y} \|^2_{2} + \| \nabla f(x^{+})  +  \gamma \delta d_{x}  +  \nabla a(x)^T (y +  \gamma d_{y}) \|^{2}_{2}.
%\label{eq:alphaD-least-squares}
%\end{flalign}

%\hinder{using stored values}

This reduces to a one-dimensional least squares problem in $\gamma$ which has a closed form expression for the solution. Equation~\eqref{eq:alphaD-least-squares} can be interpreted as choosing the step size $\alpha_{D}$ to minimize the complementarity and dual feasibility of the proximal subproblem. This encourages large dual step sizes without forgoing information about dual feasibility. We then update the new dual iterate viz
\begin{flalign}\label{eq:alpha-D}
...
$$
with $\conWeight \ge 0$ and $\vioVar^{0} > 0$. For any fixed $x^{0}$ one can always pick sufficiently large $\conWeight$ and $\mu^{0}$ such that $\mu^{0} \conWeight > a(x^{0})$, and setting $s^{0} \gets \conWeight \vioVar^{0} - a(x^{0})$ meets our requirements. For the details of how to initialize the variables in the practical implementation, see Section~\ref{sec:initialization}.

The general idea of Algorithm~\ref{one-phase-IPM} follows. At each outer iteration we factorize the matrix $\mathcal{K}_{\delta}$ with an appropriate choice of $\delta$ using Algorithm~\ref{alg:mat-fact}. With this factorization fixed, we attempt to take multiple inner iterations (at most $\parNumCor$), which corresponds to solving system~\eqref{primal-dual-Newton-direction} with different right-hand side choices, but the same matrix $\mathcal{K}_{\delta}$. Each inner iteration is either an aggressive or a stabilization step. If, on the inner iteration, the step fails (i.e. due to a too small step size), we increase $\delta$ to address this failure and refactorize $\mathcal{K}_{\delta}$. Note that we evaluate the Hessian of the Lagrangian and the full Jacobian once per outer iteration (we do one Jacobian-vector product per inner iteration). The selection of the initial point $(x^{0}, y^{0}, s^{0}, \mu^{0})$ is described in Section~\ref{sec:initialization}.
...
\textbf{Output:} some point $(x, y, s, \mu)$ that satisfies inequalities \termination{}
\vspace{0.1 cm} \\
...
\hspace*{0.1cm} \textbf{Output:} New value for $\delta$, factorization of $\mathcal{K}_{\delta}$.
...
\hspace*{0.1cm}  \textbf{Input:} $\mathcal{K}_{\delta}$, factorization of $\mathcal{K}_{\delta}$ and the point $(x,y,s, \mu)$. \\
...
\hspace*{0.1cm} \textbf{Input:} $\mathcal{K}_{\delta}, \schur$, factorization of $\mathcal{K}_{\delta}$ and the point $(x,y,s, \mu)$. \\
\hspace*{0.1cm} \textbf{Output:} A $\status$ and a new point $(x^{+},y^{+},s^{+}, \mu^{+})$.
\end{enumerate}
\item \emph{Deal with failures}. If $\status = \success$ set $(x, y, s, \mu) \gets (x^{+},y^{+},s^{+}, \mu^{+})$. If $\status = \failure$ and $j = 1$ go to \eqref{increase-delta-for-failure}.  If $\status = \failure$ and $j > 1$ go to step~\eqref{step-1}.
...
\end{enumerate}
Terminate with $\status = \failure$ if $\alpha_{P}$ satisfies \eqref{min-step-size-aggresssive}.
\end{enumerate}
...
To be precise, we accept any iterate $(x^{+}, y^{+}, s^{+}, \mu^{+})$ that makes sufficient progress on the augmented log barrier function $\phi_{\mu}$, or satisfies the two equations
...
\end{flalign}
\hinder{replace zero with $b$ etc?}
...
$\parNumCor$ & Maximum number of steps per outer iteration. See \eqref{take-steps} of Algorithm~\ref{one-phase-IPM}.  & Any natural number & $3$  \\ 
 \hline
  $\parComp$ & Restricts how far complementarity of $s$ and $y$ can be from $\mu$. See \eqref{eq:comp-slack}.  & The interval $(0,1)$ & $0.01$ \\ 
 \hline
...
\end{flalign*}
where the first inequality follows from the fact that \eqref{terminate-primal-infeasible} is not satisfied; the second inequality from $\| \nabla \Lag (x^k , y^k) \|_{\infty} = 0$ and $S^k y^k = \mu^k$; the third inequality from the assumptions that $G \ge \mu^{k}$ and $\TOLopt \le 1$.
Now,
\begin{flalign*}
