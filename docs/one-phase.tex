...
\newcommand{\parMinStableStepSize}{\beta_{6}}
...
\begin{flalign}
\min_{x \in \R^{\nvar}} \barrier_{\mu}(x) := \obj(x) + \mu r(x)  - \mu  \sum_i{ \log \left( \mu \conWeight_i - \cons_i(x)  \right) } \label{shifted-barrier-problem}
\end{flalign}
...
\regularizer(x) := \parConRegularizer \ones^T \cons(x) + \sum_{i = 1}^{\nvar} \sqrt{\parRegularizer^2 x_i^2 + 1} ,
\end{flalign}
where $\parRegularizer, \parConRegularizer \in (0,1)$ are constants with default values $\parRegularizerValue$ and $\parConRegularizerValue$ respectively. Note that as $\mu \rightarrow 0$ the impact of the regularizer $\regularizer$ diminishes and for $x_i \gg \parRegularizer$ that $\sqrt{\parRegularizer^2 x_i^2 + 1} \approx \parRegularizer | x_i |$. The regularizer enables us to guarantee that for any fixed $\mu$, if the $\obj(x)$ values generated by the algorithm are bounded below, then the iterates are bounded (Lemma~\ref{lem:compact-Q}) and thereby allows us to prove our general convergence result (Theorem~\ref{thm:global-convergence}).  Without the regularizer the primal iterates may unnecessarily diverge even when $f$ is bounded below (for examples see Appendix~\ref{app:examples-regularizer}). We remark with $\parRegularizer = 0$ and $\parConRegularizer > 0$ that this modification of the log barrier function is similar to previous works \hinder{more refs} \cite[Section 3.7]{wachter2006implementation}.
...
\missingfigure[figwidth=6cm]{Plot of shifted log barrier with $\sin$. Size of shift is $O(\mu)$}
...
\end{flalign}
where $\Lag_{\mu} (x, y) := \obj(x) + y^T \cons(x) + \mu r(x)$ is the regularized Lagrangian function and $c > 0$ is some constant. Requiring \eqref{eq:barrier-primal-sequence-nice} and \eqref{eq:dual-feas} is common in many practical linear programming implementations \cite{mehrotra1992implementation} \if\inProgress1 \cite{more} \fi. Note that \eqref{eq:barrier-primal-sequence-nice} and \eqref{eq:dual-feas} can be interpreted as a `central sequence'. This is weaker than the existence of a central path, a concept from convex optimization \cite{andersen1999homogeneous,megiddo1989pathways}. Unfortunately, in non-convex optimization there may not exist a continuous central path\if\inProgress1 (see Appendix~\ref{app:non-existence-of-central-path})\fi.
...
We remark that the direction $\dir{s}$ is not used for updating the iterates (Section~\ref{sec:update-iterates}), however, we define it for connivence.
...
\end{matrix} \right)
$$
is factorable. As we show next for a large set of perturbations system~\eqref{eq:ldl-system} is factorable.

\begin{lemma}
Consider matrices $H  \in \R^{\nvar \times \nvar}$, $A \in \R^{\nvar \times \ncon}$ and $Q \in \R^{\ncon \times \ncon}$ with $H$ symmetric. Let $M = H + A^T D^2 A$. If $M \succ 0$ and $Q \succ 0$ then for all permutation matrices $P_{1}$ and $P_{2}$ the matrix
$$
\left( \begin{matrix}
P_{1} & 0 \\
0 & P_{2}
...
\end{matrix}\right).
$$
Note that since $Q \succ 0$ this matrix is singular only if $H_{S} + A^T_{S} Q^{-1} A_{S}$ is singular (otherwise the linear system involving this principal sub-matrix has a solution for any right hand side). However, $H_{S} + A^T_{S} Q^{-1} A_{S}$ is a sub-matrix of $M$ hence $H_{S} + A^T_{S} Q^{-1} A_{S} \succ 0$.
\end{proof}

Therefore, we intend to add \LDL{} factorization (by modifying the pivot ordering of an existing \LDL{} solver) as a future option for our solver.
...
\| \grad \Lag_{\mu}(x, y) \|_{1} &\le  \max\{ \| \grad \obj(x) \|_{1} ,  m \mu / \parCompAgg \} \label{agg-criteron-farkas} \\
...
\theta(\mu, s) := \min\left\{ 1/2, \min_{i \in \{ 1, ..., \ncon \} : \conWeight_i > 0}{\frac{s_{i}}{2 \mu \conWeight_i} } \times \frac{\parCompAgg - \parComp}{\parCompAgg} \right\},
\end{flalign}
otherwise we declare the step a failure (see Line~\ref{line:too-small-step} of Algorithm~\ref{alg:simple-agg-step}). However to ensure the step is taken, on line~\ref{simple-agg-large-delta} of Algorithm~\ref{simple-one-phase} we choose a sufficiently large delta  such that $\alpha_{P}$ selected by Algorithm~\ref{alg:simple-agg-step} satisfies $\alpha_{P} \ge \theta(\mu, s)$ (such a $\delta$ must exist by Lemma~\ref{lemma:agg-succeeds}).
...
\item \label{line:too-small-step} If $\alpha_{P} \ge \theta(\mu, s)$  then $\status =\success$ otherwise $\status =\failure$.
\end{enumerate}
...
\end{flalign*}
where the first inequality holds via convexity, the second by definition of $\infeasFuncOne$, the third and fourth by assumption. Since $y \ge 0$ we deduce $a_i(x^{*}) > 0$ for some $i$.
\end{proof}
\subsection{Global convergence proofs for Algorithm~\ref{simple-one-phase}}\label{sec:global-conv}

We now give a global convergence proof for Algorithm~\ref{simple-one-phase} as stated in Theorem~\ref{thm:global-convergence}. We remark that the proof is mostly mechanical, for this reason we place most of it in Appendix~\ref{app:global-conv}. 

\begin{assumption}\label{assume-convergence} Assume the following.
\begin{enumerate} 
\item \label{assume:differentiable} The functions $f : \R^{\nvar} \rightarrow \R$ and $a : \R^{\nvar} \rightarrow \R^{\ncon}$ are twice differentiable on $\R^{\nvar}$.
\item \label{assume:G} There exists some constant $G \ge \max\{ m / \parComp, \| \grad \obj(x) \|_{1}  \}$ for all $x \in \{ x \in \R^{\nvar} :  a(x) < \conWeight \mu^0\}$. 
\item \label{assume:tol} The tolerance $\TOLall \in (0,1)$ and $\TOLall \le \min\{ \TOLopt, \TOLinfOne, \TOLinfTwo, \TOLunbounded \}$.
\end{enumerate}
\end{assumption}

\subsubsection{Convergence of aggressive steps}
The goal of this section is show that after a finite number of aggressive steps, Algorithm~\ref{simple-one-phase} converges. Lemma~\ref{lemma:agg-succeeds} uses the fact that for large enough $\delta$ the slack variables can be absorbed to reduce the barrier parameter $\mu$. In particular, Lemma~\ref{lem:agg-finite} uses the infeasibility termination criterion~\eqref{terminate-primal-infeasible} to bound the dual variables and uses that to show that successful aggressive steps will converge.
...
Assume the functions $f : \R^{\nvar} \rightarrow \R$ and $a : \R^{\nvar} \rightarrow \R^{\ncon}$ are twice differentiable on $\R^{\nvar}$. Consider any point $(\mu, x, s, y)$ that satisfies the criterion for an aggressive step \eqref{agg-criteron} with $\cons(x) + s = \mu \conWeight$ and $s, y, \mu > 0$, then there exists some $\bar{\delta}$ such that for all $\delta > \bar{\delta}$ Algorithm~\ref{alg:simple-agg-step} returns $\status = \success$.
\end{restatable}

The proof of Lemma~\ref{lemma:agg-succeeds} is given in Section~\ref{sec:lemma:agg-succeeds}.
Next, we show that, for a infeasibility termination criterion, the term $y^T \conWeight$, remains bounded. Consequently, we know the algorithm will terminate in a finite number of steps.

\begin{restatable}{lemma}{lemAggFinite}\label{lem:agg-finite}
Suppose Assumption~\ref{assume-convergence} holds.
...
$$
y^T \conWeight \le \frac{2 G}{\TOLall^2}.
$$
Furthermore, after a finite number of calls to Algorithm~\ref{alg:simple-agg-step}, Algorithm~\ref{simple-one-phase} will terminate.
\end{restatable}

The proof of Lemma~\ref{lemma:agg-succeeds} is given in Section~\ref{sub:lem:agg-finite}.
However, Lemma~\ref{lem:agg-finite} does not rule out the possibility there is an infinite number of consecutive stabilization steps. Ruling out this possibility is the purpose of subsection~\ref{conv:stb}.
...
Assume the functions $f : \R^{\nvar} \rightarrow \R$ and $a : \R^{\nvar} \rightarrow \R^{\ncon}$ are twice differentiable on the set $\R^{\nvar}$. For any constants $C, \mu > 0$ the set $\mathbb{Q}_{\mu, C}$ is compact.
\end{restatable}

The proof of Lemma~\ref{lem:compact-Q} is given in Section~\ref{sec:lem:compact-Q} and is dependent on the regularizer $\regularizer$, since without it the set $\mathbb{Q}_{\mu, C}$ could be unbounded (see Appendix~\ref{app:examples-regularizer}). They key argument is 
$$
...
$$
which allows us to conclude that $\| x \|$ is bounded from above.
...
The proof of Corollary~\ref{coroConsecutiveStable} is given in Section~\ref{sec:coroConsecutiveStable}. Let us sketch the main ideas. Recall that a continuous function on a compact set has a supremum. Consequently, Lemma~\ref{lem:compact-Q} allows us to uniformly bound quantities such as Lipschitz constants. The crux of the proof is showing if there is cannot be a infinite number of consecutive stabilizations steps because this will imply the augmented log barrier merit function tends towards negative infinity and the unboundedness termination criterion holds.
%Since both the augmented log barrier function and KKT error are bounded below contradiction
...
Suppose Assumption~\ref{assume-convergence} holds, then Algorithm~\ref{simple-one-phase} terminates after a finite number of iterations.
\end{theorem}

\begin{proof}
Lemma~\ref{lem:agg-finite} shows that the algorithm must terminate after a finite number of aggressive steps. Corollary~\ref{coroConsecutiveStable} shows that the algorithm terminates or an aggressive step must be taken after a finite number of stabilization steps. The result follows.
\end{proof}
...
s^{+} \ge  \parFracBoundary \min\{ s, (\delta + \| \dir{x} \|_{\infty} )  \| \dir{x} \|_{\infty}  e \},
\end{flalign}
$$
\Theta^{p}, \Theta^{b}
$$

where $\parFracBoundary \in \parFracBoundaryInterval$ with default value $\parFracBoundaryValue$. Note that the $\| \dir{x} \|^2_{\infty}$ term plays a similar role to $\mu$ in the more typical $s^{+} \ge  \parFracBoundary s \min\{ 1 , \mu \}$ fraction to the boundary rule (say of IPOPT \cite{wachter2006implementation}) in that it allows (almost) unit steps in the super-linear convergence regime.
...
s + \alpha_{P} \dir{s} &\ge  \parFracBoundaryMax  \min\{ s, \max\{ 2 \| \dir{x} \|_{\infty} (\delta + \| \dir{x} \|_{\infty} ) , \| \dir{x} \|_{\infty}^{\parFracBoundaryExp} \} \} 
\end{flalign}
...
$$
\parFracBoundary \min\{ s, \| \dir{x} \|_{\infty} (\delta + \| \dir{x} \|_{\infty} ) \} < \parFracBoundaryMax  \min\{ s, \max\{ \| \dir{x} \|_{\infty} (\delta + \| \dir{x} \|_{\infty} ) , \| \dir{x} \|_{\infty}^{\parFracBoundaryExp} \} \} 
$$
...
\inProgressHide{
\subsection{Line search with quadratic approximation of constraints}

\hinder{maybe add to reduce number of constraint evaluations}

$$
\tilde{s}(\eta) = s + \eta v + \eta^2 u
$$

$$
u = \frac{s^{+} - s - \dir{s} \alpha_{P}}{\alpha_{P}^2}
$$
...
$$
\eta^{*} \gets \frac{\dir{s}^2 \pm \sqrt{u(s-l) - 4 \dir{s}^2}}{2}
$$

$$
\alpha_{P} \gets \min\{0.5 \alpha_{P}, \eta^{*} \}
$$
}
...
\bar{\theta}(\mu, s) = \min \left\{ 1/2, \min_{i \in \{ 1, ..., \ncon \} : \conWeight_i > 0}{\frac{\parBacktracking \theta s_{i}}{2 \mu \conWeight_i} } \times \min\left\{ \frac{\parCompAgg - \parComp}{\parCompAgg}, 1 - \parFracBoundary \right\} \right\}
...
\item \label{line:agg-back-too-small} \emph{Check the step size is not too small.} If $\alpha_{P} \le \bar{\theta}(\mu, s)$ then goto line~\ref{line:agg-terminate}.
...
\| \cons(x) - \cons(x + \alpha_{P} \dir{x}) \|_{\infty} \le \frac{\theta}{2} \min\{ s_{i} \}
\end{flalign}
...
$$
\| s^{+} - s \|_{\infty} = \|  -\alpha_{P} \mu\conWeight+  \cons(x) - \cons(x + \alpha_{P} \dir{x}) \|_{\infty} \le  \alpha_{P} \mu \|  \conWeight \|_{\infty} +  \| \cons(x) - \cons(x + \alpha_{P} \dir{x}) \|_{\infty} \le \theta \min_i\{ s_{i} \},
$$
...
\begin{flalign}\label{eq:alpha-bound}
\alpha_{P} = \theta(\mu, s) = \min_{i : w_i > 0}{ \frac{(\parCompAgg - \parComp) s_{i}}{2  \parCompAgg \mu \conWeight_i} } 
\end{flalign}
then for this choice of $\alpha_{P}$ we have
$$
...
Therefore $\alpha_{D} = 0$ gives a feasible dual iterate. We conclude there exists a $\delta$ such that the $\alpha_{P}$ chosen by line~\ref{simple-agg-select-alpha-P} is at least $\theta(\mu, s)$, which proves the result.
\end{proof}
...
$$
Q := \left\{ x \in \R^{\nvar} : (y, s) \in \R^{\ncon++} \times \R^{\ncon++}, \phi_{\mu}(x,y,s) \le C, f(x) \ge -C \right\} 
$$
Now, consider some $x \in Q$. Recall that
$$
\phi_{\mu}(x, s, y) = \obj(x) + \mu \left( \parRegularizer \sum_{i = 1}^{\nvar} \sqrt{x_i^2 + 1 / \parRegularizer^2} - \parConRegularizer \ones^T \cons(x) \right)  - \mu  \sum_i{ \log \left( \mu \conWeight_i - a_i(x)  \right) } + \frac{\| S y - \mu \|_{\infty}^3}{\mu^2}.
$$
The expression $\parConRegularizer a_i(x) - \log(\mu \conWeight_i - a_i(x))$ and $\frac{\| S y - \mu \|^3}{\mu^2}$ are bounded from below. Therefore there exists some constant $K_{1} > 0$ such that
$$
-C \le \obj(x) \le K_{1} - \parRegularizer \sum_{i}{\sqrt{x_i^2 + 1 / \parRegularizer^2}}
$$
...
\end{flalign*}
Since $\max\{ \MeritComp_{\mu}(s,y), \| \grad \barrier_{\mu}(x)\| \}$ is bounded away from zero, we deduce the largest $\alpha_{P}$ satisfying $\phi_{\mu}(x^{+},y^{+},s^{+}) - \phi_{\mu}(x,y,s) \le \Upsilon(\alpha_{P})$ is bounded away from zero.
\end{proof}
