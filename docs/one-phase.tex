...
Consider a point $(\mu, x, s, y)$ that satisfies the criterion for an aggressive step \eqref{agg-criteron}, but does not satisfy the infeasibility termination criterion~\eqref{terminate-primal-infeasible} or the unboundness criterion~\eqref{terminate-dual-infeasible}. Then, after a finite number of calls to Algorithm~\ref{alg:simple-agg-step}, Algorithm~\ref{simple-one-phase} will terminate.
\end{restatable}

The proof of Lemma~\ref{lem:agg-finite} is given in Section~\ref{sub:lem:agg-finite}.
...
\bar{\minStepFunc}(\mu, s) = \min \left\{ 1/2, \min_{i \in \{ 1, ..., \ncon \} : \conWeight_i > 0}{\frac{\parBacktracking s_{i}}{4 \mu \conWeight_i} } \times \min\left\{ \frac{\parCompAgg - \parComp}{\parCompAgg}, 1 - \parFracBoundary \right\} \right\}
...
\begin{proof}
Assume that \eqref{terminate-primal-infeasible} does not hold. In this case either $a(x)^T y \le 0$,
...
$$
w^T y \le m / \parComp + \frac{\| \grad  a(x)^T y \|_{1}}{ \TOLinfOne  } \le m / \parComp + \frac{\| \nabla f(x) \|_{1} + \mu \| \grad r(x) \|_{1} + \| \grad \Lag_{\mu}(x,y) \|_{1}}{ \TOLinfOne  }
$$
??????????
...
\begin{flalign*}
y^T \conWeight &\le \frac{\max\{ \| \grad \cons(x)^T y \|_{\infty}, \| S y \|_{\infty} \}}{ \TOLinf \min\{  1, \mu \}} \\
& \le  \frac{\max\{\| \grad \obj(x) \|_{\infty} + \| \grad \Lag_{\mu} (x , y) \|_{\infty}, \mu / \parComp \}}{ \TOLinf \min\{  1, \mu \}} \\
& \le \frac{ 2 G }{\TOLall^2} 
\end{flalign*}
