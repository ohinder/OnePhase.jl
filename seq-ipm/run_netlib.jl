...
function run_netlib_problems_using_IPOPT(problem_list::Array{String,1}, test_name::String, bound_relax_factor::Float64,mehrotra_algorithm::String)
    max_iter = 100
    ...
    for problem_name in problem_list
        ...
        solver = IpoptSolver(print_level=5, max_iter=max_iter, tol=1e-6, max_cpu_time=60.0 * 60.0, nlp_scaling_method="none", bound_relax_factor = 0.0, acceptable_iter=999999, mehrotra_algorithm=mehrotra_algorithm) #, mumps_pivtol=0.1)
        ...
        if status == :Optimal
          hist = run_IPOPT(A, b, c, lbounds, ubounds, max_iter, bound_relax_factor, mehrotra_algorithm)
...
problem_list = load_netlib(Int(3e3))
...
begin
  ...
  if false
  test_name = "netlib/well-behaved-2"
  ...
  test_name = "netlib/ipopt-mehrotra"
  run_netlib_problems_using_IPOPT(problem_list, test_name, 0.0, "yes")
  end
end
