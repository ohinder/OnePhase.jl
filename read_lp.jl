...
function run_IPOPT(A, b, c, lbounds, ubounds, max_iter, bound_relax_factor)
    m = read_lp_into_JuMP(A, b, c, lbounds, ubounds, 0.0, true)
    nlp_raw = MathProgNLPModel(m)
    return run_IPOPT(nlp_raw, max_iter, bound_relax_factor)
end
function run_IPOPT(nlp_raw, max_iter, bound_relax_factor)
  t = 0
  hist = Array{ipopt_alg_history,1}()

  println(pd("t"), pd("comp"), pd("prm"), pd("dual"), pd("|x|"), pd("|y|"))
  A = jac(nlp_raw,zeros(length(b)))
  ...
  for t = 1:max_iter
    ...
    solver = IpoptSolver(print_level=2, acceptable_iter=t, acceptable_tol=Inf, acceptable_compl_inf_tol=Inf, acceptable_constr_viol_tol=Inf, tol=1e-6, bound_relax_factor=bound_relax_factor)
    #setsolver(m, solver)
    #solve(m)
