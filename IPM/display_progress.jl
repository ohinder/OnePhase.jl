...
function head_progress()
  println( pd("it",5), pd("step", 5), pd("eta"), pd("α_P"), pd("α_D"), pd("ls",3), pd("|dx|"), pd("|dy|"), pd("N err"), "|", pd("mu"), pd("dual"), pd("primal"), pd("cmp scaled"), pd("inf"), "|", pd("delta"), pd("|x|"), pd("|y|"), pd("∇phi"), pd("phi") )
end
